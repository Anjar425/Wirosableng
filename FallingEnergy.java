 ...
 */
public class FallingEnergy extends FallingObject
{
    /**
    ...
    public void update() {
        ...
        animate(energy, 200, true);
        if (!beenClicked) {
            if (checkClick()) {
                beenClicked = true;
                AudioPlayer.play(90, "points.mp3");
                MyWorld.heroBank.energycounter.addEnergy(25);
        ...
        if (!beenClicked) {
            ...
            if (!(Math.abs(getX()-EnergyCounter.x) < 20 && Math.abs(getY()-EnergyCounter.y) < 20)) {
                turnTowards(EnergyCounter.x,EnergyCounter.y);
                move(20);
            }
        }
        checkDeath();
